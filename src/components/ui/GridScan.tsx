...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (
        ...
        (window as Window & typeof globalThis & { DeviceOrientationEvent?: { requestPermission?: () => Promise<string> } }).DeviceOrientationEvent &&
        (window as Window & typeof globalThis & { DeviceOrientationEvent?: { requestPermission?: () => Promise<string> } }).DeviceOrientationEvent?.requestPermission
      ) {
        try {
          await (window as Window & typeof globalThis & { DeviceOrientationEvent: { requestPermission: () => Promise<string> } }).DeviceOrientationEvent.requestPermission();
        } catch {}
      }
    };
  ...
  // Store derived values in refs to avoid dependency issues
  const skewScaleRef = useRef(skewScale);
  const tiltScaleRef = useRef(tiltScale);
  const yawScaleRef = useRef(yawScale);
  const smoothTimeRef = useRef(smoothTime);
  const maxSpeedRef = useRef(maxSpeed);
  const yBoostRef = useRef(yBoost);

  // Update refs when sensitivity changes
  useEffect(() => {
    skewScaleRef.current = skewScale;
    tiltScaleRef.current = tiltScale;
    yawScaleRef.current = yawScale;
    smoothTimeRef.current = smoothTime;
    maxSpeedRef.current = maxSpeed;
    yBoostRef.current = yBoost;
  }, [skewScale, tiltScale, yawScale, smoothTime, maxSpeed, yBoost]);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      lookCurrent.current.copy(
        smoothDampVec2(lookCurrent.current, lookTarget.current, lookVel.current, smoothTimeRef.current, maxSpeedRef.current, dt)
      );
  ...
  useEffect(() => {
    ...
    if (bloomRef.current) {
      ...
      const bloomEffect = bloomRef.current as unknown as {
    ...
    const start = async () => {
      ...
      const detect = async (ts: number) => {
        ...
        const videoEl = video as HTMLVideoElement & {
          requestVideoFrameCallback?: (callback: (now: number) => void) => void;
        };
...
function smoothDampFloat(
  ...
  const maxChange = maxSpeed * smoothTime;
  const clampedChange = Math.sign(change) * Math.min(Math.abs(change), maxChange);
  target = current - clampedChange;
  const temp = (velRef.v + omega * clampedChange) * deltaTime;
  velRef.v = (velRef.v - omega * temp) * exp;
  let out = target + (clampedChange + temp) * exp;
